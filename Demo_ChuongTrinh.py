...
class MAHOA_RSA(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.lb2 = Label(self, text="MẬT MÃ BẤT ĐỐI XỨNG RSA",
                         font=("Arial Bold", 15))
        self.lb2.grid(column=1, row=2)
    ...
    def giaima_rsa(self):
        ...
        detxt = cipher.decrypt(base64.b64decode(self.ciphertxt.get()), "Lỗi khi giải mã RSA").decode()
        self.denctxt.delete(0, END)
        self.denctxt.insert(INSERT, detxt)


class MAHOA_Ceasar(tk.Toplevel):
    def __init__(self, parent):
        self.parent = parent
        Toplevel.__init__(self)
        self.title("Chương trình mã hóa đối xứng")
        ...
        self.lb2 = Label(self, text="MẬT MÃ CAESAR", font=("Arial Bold", 15))
        self.lb2.grid(column=0, row=2)
    ...
    def mahoa(self):
        k = int(self.KEYA1.get())
        m = 26

        entxt = ""
        txt = self.plaintxt.get()
        for c in txt:
            if (c.isupper()):
                c = Char2Num(c)
                c = (c+k)%m
...
class MainWindow(tk.Frame):
    def __init__(self, parent):
        ...
        self.mahoa_Caesar = Button(
            text="Mã hóa Caesar",
            font=("Times New Roman", 11),
            command=self.caesar
        )
        self.mahoa_Caesar.pack()
